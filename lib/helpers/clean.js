...
const dataClean = (dataObject) => {
  ...
  const currentData = {
      ...
      date: dataObject.current_observation.local_time_rfc822,
  ...
  const hourlyData = () => {
    if (dataObject.hourly_forecast) {
      return dataObject.hourly_forecast
      .slice(0, 7)
      .map(obj => {
        return ({
          time: obj.FCTTIME.pretty,
          icon: obj.icon,
  ...
  const tenDay = () => {
    if (dataObject.forecast.simpleforecast.forecastday) {
