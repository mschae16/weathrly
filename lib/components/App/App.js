...
export default class App extends Component {
  ...
  render() {
    return (
      <div>
        {
          ((localStorage.getItem('location')) === null) && (!this.state.error) &&
...

        {
          ...
          <div className="error-display">
            <Welcome
              lost={<img className='lost-img' src='assets/lost-logo.svg' alt="Lost logo"/>}
              container={'Error-container'}
              heading={'Error-heading'}
              search={'Error-search'}
              input={'Error-input-field'}
              dropdown={'Drop-down-error'}
              button={'Error-button'}
              onClick={this.handleSearch.bind(this)} />
          </div>
        }
        ...
        { (this.state.location) && (!this.state.error) &&
          <div className="weather-display">
            ...
            <CurrentWeather
              temperature={Math.floor(this.state.currentData.temperature)}
